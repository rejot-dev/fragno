...
export default function BackofficeOrganisationPiSessionsLayout() {
  ...
  const createSessionPanel = (
    <div className="border border-[color:var(--bo-border)] bg-[var(--bo-panel-2)] p-3">
      ...
      {harnesses.length === 0 ? (
        ...
        <Form method="post" action={basePath} className="mt-4 space-y-3">
  ...
  return (
    <section className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(280px,420px)_minmax(0,1fr)]">
      <div
      ...
      >
        ...
        <div className={`min-h-0 flex-1 overflow-auto pr-1${isNewSession ? "" : "mt-3"}`}>
          {sessions.length === 0 ? (
      ...
      <div
        className={`${detailVisibility} flex min-h-0 flex-col border border-[color:var(--bo-border)] bg-[var(--bo-panel)] p-4`}
      >
        <Outlet
          context={{
            sessions,
