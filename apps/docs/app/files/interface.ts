...
export interface IIFileSystem {
  ...
  /**
   * Read the contents of a file as a stream when the backend can provide one natively.
   *
   * This is intentionally optional so callers can prefer streaming for large downloads while
   * falling back to guarded buffered reads for backends that only support whole-file reads.
   * @throws Error if file doesn't exist or is a directory
   */
  ...
  /**
   * Get known paths in the filesystem (useful for glob matching).
   *
   * Results are advisory and may be sparse. Implementations may omit paths they do not
   * know about cheaply, so callers must not rely on this list being exhaustive.
   */
  getAllPaths(): string[];
