...
describe("static file contributor", () => {
  ...
  test("renders and reads the built-in /system docs pack", async () => {
    ...
    const readme = await staticFileContributor.readFile?.(`${STATIC_FILE_MOUNT_POINT}/README.md`);

    expect(entries?.map((entry) => entry.name)).toEqual(
      expect.arrayContaining(["README.md", "roots"]),
    );
    ...
    expect(readme).toContain("Fragno Files workspace");
  });
});
