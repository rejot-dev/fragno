...
can be greatly if it was easier for the service provider to offer full-stack integration primitives.
By distributing frontend, backend, and database primitives you shift much of the integration work to
the service provider.

A common problem when integrating with a third party is the split brain problem: a copy of the
service provider's data is replicated to the customers application and this data must be kept in
sync. This requires that the customer is aware of the intricate details on how to do that. A
smoother integration experience would be if most of the integration code, i.e. webhook handlers and
database schemas, were provided by the service provider directly.

In this case study, we explore these ideas by implementing a Stripe subscription library that
provides components for every layer of the stack and we see how it makes integrating with Stripe
...
end users might use. This is how we built our case study Stripe integration.
