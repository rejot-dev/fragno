...
- `GET /oauth/:provider/callback` to exchange the code, issue a credential, and set the
  `fragno_auth` cookie
...
This requires a valid `fragno_auth` cookie or `Authorization: Bearer <token>` on the authorize
request.
