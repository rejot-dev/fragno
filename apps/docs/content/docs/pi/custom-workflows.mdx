...
Inside each `step.do(...)`, restore the workflow-backed Pi session, create a real `AgentHarness`,
and call its APIs directly. This gives the workflow full control over operation names, active tools,
skills, prompt templates, compaction, and tree navigation.
