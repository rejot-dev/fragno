...
```ts
import { createS3CompatibleStorageAdapter } from "@fragno-dev/fragment-upload";
...
```ts
import { createR2StorageAdapter } from "@fragno-dev/fragment-upload";
...
```ts
import { createFilesystemStorageAdapter } from "@fragno-dev/fragment-upload";
