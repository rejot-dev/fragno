...
<Callout title="Need Persistent Storage?" type="info">
  ...
  Integration](./features/database-integration) feature. It provides a type-safe ORM and lets users
...
export function createTodos(config: TodosConfig, options: FragnoPublicConfig = {}) {
  return createFragment(todosDefinition, config, [getTodosRoute, addTodoRoute], options);
}
```
