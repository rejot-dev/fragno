...
Pass `onWorkflowRestarted` and `onWorkflowTerminal` in the fragment config to maintain projections
across multiple runs of the same public instance:
...
const fragment = createWorkflowsFragment(
  {
    ...
    onWorkflowTerminal: async ({ instanceId, runGeneration, status }) => {
      await markWorkflowRunTerminal({ instanceId, runGeneration, status });
    },
  },
  { databaseAdapter: adapter },
);
...
A new instance starts at run generation `1`; every restart increments it. Terminal callbacks
describe the run that became terminal, so an older terminal hook remains valid even if it is
delivered after a restart. A failed-step retry stays in the same generation and can produce another
terminal callback, so treat callbacks within one generation as transition notifications and read the
instance when current state matters. Both callbacks execute through durable hooks, so run a
dispatcher and make their effects idempotent.
...
[Workflow Testing](/docs/workflows/testing).
...
- Publish and receive live data with
