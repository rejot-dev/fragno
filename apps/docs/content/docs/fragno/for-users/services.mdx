...
export async function signIn(formData: FormData) {
  ...
  // Extract Set-Cookie header and apply it if your framework requires manual cookie forwarding.
  const setCookie = response.headers.get("Set-Cookie");
  if (setCookie) {
    const cookieStore = await cookies();
    // Simplified example: in a real app, parse the Set-Cookie header before re-setting it.
    cookieStore.set("fragno_auth", setCookie);
  }

  const data = await response.json();
  // Auth routes now expose the issued credential as data.auth.token.
  return data;
}
```
