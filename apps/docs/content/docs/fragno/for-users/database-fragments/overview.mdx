...
output format that matches your workflow:

- **[SQL migrations](/docs/fragno/for-users/database-fragments/kysely)** - default `sql` output
- **[Drizzle schema](/docs/fragno/for-users/database-fragments/drizzle)** - `drizzle` output
- **[Prisma schema](/docs/fragno/for-users/database-fragments/prisma)** - `prisma` output

Pick the format that fits your migration tooling (SQL, Drizzle, or Prisma).
...
<Callout title="Direct deps.db Access" type="warning">
  When `shardingStrategy` is enabled, direct `deps.db` access is disabled. Use `handlerTx` or
  `serviceTx` so shard filtering is enforced.
</Callout>
