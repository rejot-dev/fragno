...
The SQL adapter integrates Fragno database Fragments with your Prisma workflow.
...
<Callout type="info" title="SQLite Storage Profile">
  For SQLite, set `sqliteProfile: "prisma"` to use Prisma-native storage for `DateTime` and
  `BigInt`. If you need the legacy Fragno SQLite storage layout, pass `sqliteProfile: "default"`.
</Callout>
