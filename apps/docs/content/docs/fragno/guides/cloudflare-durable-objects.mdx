...
## Extras: Calling DO-Hosted Routes

When a fragment runs inside a Durable Object, your app typically only has a DO stub (`fetch`) and
not a fragment instance. Use `createRouteCaller` to build a typed caller that mirrors
`fragment.callRoute()` but targets the Durable Object.

```typescript title="app/fragno/auth-server.ts"
...
`createRouteCaller` handles path params, query params, headers, request bodies, and Fragno response
parsing (including `jsonStream`).
