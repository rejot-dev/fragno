...
5. Use `step.do` + `tx.mutate` (and `tx.onTerminalError.mutate` when needed) for atomic writes.
...
If you also need to project terminal failures into your fragment schema, use
`tx.onTerminalError.mutate(...)`. That callback only commits when the step fails terminally, so it
is safe for “mark failed” or “record failed attempt” projections without firing on retryable
suspensions.

```ts title="src/workflows.ts"
await step.do("calculate-total", { retries: { limit: 1, delay: "5 s" } }, (tx) => {
  tx.onTerminalError.mutate((ctx) => {
    ctx.forSchema(myFragmentSchema).create("failed_step", {
      sessionId: event.payload.sessionId,
...
The `example-fragments/workflow-usage-fragment` example uses this pattern to persist a terminal
calc-step marker when a DSL expression fails.
