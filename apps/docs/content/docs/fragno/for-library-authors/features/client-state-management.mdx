...
to expose to the user on the client-side. Pass the object through the framework-specific adapter,
such as `createFragnoReactClient` for React or `useFragno` for Vue, Svelte, Solid, and vanilla
JavaScript, to expose reactive hooks and stores for that framework.
...
The `createHook` method takes an optional object with the following options:

- `transform`: A function to transform the response data before it reaches the user
- `refetchInterval`: Interval in milliseconds to automatically refetch data
...
<Callout title="`createStore` nesting" type="warning">
  ...
  case of a function field, nothing will happen to it. Deeply nested keys will **not** be
  considered.
</Callout>

### Parameterized Stores

`createStore` can also take a callback instead of a static object. This is useful when you need a
per-consumer store instance, for example a session controller tied to a specific `sessionId`.

```typescript title="src/index.ts"
return {
  useSession: builder.createStore(({ path }: { path: { sessionId: string } }) => {
    const controller = createSessionController(path.sessionId);

    return {
      state: controller.state,
      sendMessage: controller.sendMessage,
      [Symbol.dispose]: controller.destroy,
    };
  }),
};
```

In framework adapters, the callback is invoked per usage and top-level Nanostore fields are made
reactive just like with the static-object form.

If the returned object implements `Symbol.dispose`, Fragno will call it automatically when the
framework instance is cleaned up (for example on React unmount or when the callback arguments
change).
...
- `{ type: "options", options: RequestInit }` - Merge RequestInit options with user config
- `{ type: "function", fetcher: typeof fetch }` - Provide custom fetch function

User configuration takes precedence: custom functions override everything, RequestInit options deep
merge (user wins conflicts), headers merge with user values winning.
