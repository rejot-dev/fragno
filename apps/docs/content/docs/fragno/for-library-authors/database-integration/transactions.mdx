...
- **Handlers** call `executeRetrieve()` / `executeMutate()` and should catch errors from these calls
- **Services** await `retrievalPhase` / `mutationPhase` to coordinate their logic, but don't catch
  errors
- If an execute method throws, the handler catches it; service promises waiting on phase execution
  are abandoned
