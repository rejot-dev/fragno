...
### Retrying unique mutation races

Concurrent transactions can both observe a unique value as available before one create or update
claims it. Mark that mutation as retryable so the unique-constraint race retries the complete
transaction:

```typescript
createEvent: function (eventId: string, payload: EventPayload) {
  return this.serviceTx(mySchema)
    .retrieve((uow) =>
      uow.findFirst("events", (b) =>
    ...
    .mutate(({ uow, retrieveResult: [existing] }) => {
      ...
      uow.create("events", { id: eventId, payload }, {
...
Updates opt in through their builder and can combine this behavior with row-version checking:
...
uow.update("users", user.id, (b) =>
  b
    .set({ slug })
    .check()
    .retryOnUniqueConflict(({ error }) => error.constraint === "users_slug_idx"),
);
```

The callback receives the normalized `DatabaseConstraintError` and logical mutation context. Return
`true` only when the retrieval phase checks that unique key and resolves the conflict on retry;
returning `false` preserves the original constraint error. Constraint metadata varies by database,
so fail closed when the callback cannot identify the intended constraint. `.check()` protects the
updated row's version, while `.retryOnUniqueConflict(...)` handles the accepted unique race.

### Standalone `uow.check(table, id)`

Use a standalone check to guard a row without updating it:
...
### Assert absence with `uow.checkAbsent()`

Use a complete, non-null unique-index key to assert that no matching row exists:
...
The unique constraint remains the final guard against a concurrent create after the check.

### Important: `.check()` requires a `FragnoId`, not a string

- ✅ `uow.check("users", user.id)`
- ❌ `uow.check("users", "user-123")`

Updates and deletes can use a string ID without `.check()`, but then use last-write-wins semantics.
