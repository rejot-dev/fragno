...
```typescript
// Basic types
column("string"); // Short string, maps to VARCHAR(191)
column("text"); // Unbounded text; not indexable
column("integer");
column("boolean");
column("timestamp");
column("json");
...
<Callout title="Not all types are indexable" type="info">
  Because of limitations in a number of database systems, not all types are indexable. `json`,
  `binary`, and `text` are not indexable. `column("string")` is intended for indexable short
  strings, while `column("text")` cannot be indexed.
</Callout>
...
- Learn about [Querying](/docs/fragno/for-library-authors/database-integration/querying) with the DB
- See
