...
export const baseArgs = {
  "base-url": {
    type: "string",
    short: "b",
    description: "Workflow fragment base URL (env: FRAGNO_WF_BASE_URL)",
  },
  header: {
    type: "string",
    short: "H",
    description: "Extra HTTP header (repeatable), format: 'Name: value' (env: FRAGNO_WF_HEADERS)",
    multiple: true,
  },
  timeout: {
    type: "number",
    description: "Request timeout in ms (env: FRAGNO_WF_TIMEOUT_MS, default: 15000)",
  },
  retries: {
    type: "number",
    description: "Retry count for network/5xx/429 (env: FRAGNO_WF_RETRIES, default: 2)",
  },
  "retry-delay": {
    type: "number",
    description: "Delay between retries in ms (env: FRAGNO_WF_RETRY_DELAY_MS, default: 500)",
  },
} as const;
...
const parseEnvHeaders = () => {
  const envHeaders = process.env["FRAGNO_WF_HEADERS"];
  if (!envHeaders) {
    return [];
  }
  return envHeaders
...
const resolveTimeout = (ctx: CommandContext) =>
  ...
  parseNumberEnv(process.env["FRAGNO_WF_TIMEOUT_MS"]) ??
  DEFAULT_TIMEOUT_MS;

const resolveRetries = (ctx: CommandContext) =>
  (ctx.values["retries"] as number | undefined) ??
  parseNumberEnv(process.env["FRAGNO_WF_RETRIES"]) ??
  DEFAULT_RETRIES;

const resolveRetryDelay = (ctx: CommandContext) =>
  (ctx.values["retry-delay"] as number | undefined) ??
  parseNumberEnv(process.env["FRAGNO_WF_RETRY_DELAY_MS"]) ??
  DEFAULT_RETRY_DELAY_MS;

export const resolveBaseUrl = (ctx: CommandContext) => {
  const baseUrl =
    (ctx.values["base-url"] as string | undefined) ?? process.env["FRAGNO_WF_BASE_URL"];
  if (!baseUrl) {
    throw new Error("Missing base URL. Provide --base-url or set FRAGNO_WF_BASE_URL.");
  }
