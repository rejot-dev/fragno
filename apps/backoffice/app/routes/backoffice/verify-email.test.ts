...
describe("backoffice email verification route", () => {
  ...
  test("loads confirmation input without mutating during GET", () => {
    expect(
      loader(
        createLoaderArgs(
    ...
    ).toEqual({ state: "ready", userId: "user_123", code: "ABC12345" });
    expect(getSystemOtpDurableObject).not.toHaveBeenCalled();
  });

  test("rejects incomplete verification links", () => {
    expect(loader(createLoaderArgs("https://example.com/backoffice/verify-email"))).toEqual({
      state: "result",
      result: "incomplete",
    });
  });

  test("delegates verification completion to the OTP authority", async () => {
  ...
  test("requests a replacement verification email", async () => {
