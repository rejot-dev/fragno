...
const marketplacePagePath = (input: { category?: string; cursor?: string }) => {
  ...
  return query ? `/backoffice/marketplace?${query}` : "/backoffice/marketplace";
};
...
export default function BackofficeMarketplace({ loaderData }: Route.ComponentProps) {
  const { category, listings, nextCursor, hasNextPage } = loaderData;

  return (
    <div className="space-y-4">
      <BackofficePageHeader
        breadcrumbs={[{ label: "Backoffice", to: "/backoffice" }, { label: "Marketplace" }]}
        eyebrow="Automation exchange"
        title="Discover reusable automations."
        description="Browse published marketplace metadata. Version artifacts will be attached through named uploads later."
        actions={
      ...
      {hasNextPage && nextCursor ? (
        <div className="flex justify-end">
          <Link
            to={marketplacePagePath({ category: category ?? undefined, cursor: nextCursor })}
            className="border border-[color:var(--bo-border)] bg-[var(--bo-panel-2)] px-4 py-2 text-[10px] font-semibold tracking-[0.22em] text-[var(--bo-muted)] uppercase transition-colors hover:border-[color:var(--bo-border-strong)] hover:text-[var(--bo-fg)]"
          >
            Next shelf →
          </Link>
