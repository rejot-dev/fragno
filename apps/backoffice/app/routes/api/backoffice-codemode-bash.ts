...
import type { Route } from "./+types/backoffice-codemode-bash";
...
class BackofficeBashCommandTimeoutError extends Error {
  constructor(timeoutMs: number) {
    super(`Command timed out after ${timeoutMs}ms.`);
    this.name = "BackofficeBashCommandTimeoutError";
  }
}
...
export async function action({ request, context, params }: Route.ActionArgs) {
  ...
  try {
    const result = await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeoutId = setTimeout(() => {
          ...
          reject(new BackofficeBashCommandTimeoutError(timeoutMs));
        }, timeoutMs);
      }),
    ]);
    ...
    const durationMs = Math.round(performance.now() - startedAt);
    const timeout = error instanceof BackofficeBashCommandTimeoutError;
