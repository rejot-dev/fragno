...
const defaultLogger: Logger = {
  info: (...data: any[]) => console.info(LOG_PREFIX, ...data),
  error: (...data: any[]) => console.error(LOG_PREFIX, ...data),
  warn: (...data: any[]) => console.warn(LOG_PREFIX, ...data),
  debug: (...data: any[]) => console.debug(LOG_PREFIX, ...data),
  log: (...data: any[]) => console.log(LOG_PREFIX, ...data),
};
...
export const stripeFragmentDefinition = defineFragmentWithDatabase<StripeFragmentConfig>("stripe")
  ...
  .withServices(({ deps, orm }) => {
    return {
      ...createStripeServices(deps, orm),
    };
  });

function createStripeServices(
  deps: StripeFragmentDeps,
  orm: AbstractQuery<typeof stripeSchema>,
): StripeFragmentServices {
  const services: StripeFragmentServices = {
    ...
    createSubscription: async (
      ...
      return (await orm.create("subscription", data)).externalId;
    },
    updateSubscription: async (
      id: string,
      data: Partial<Omit<TableToInsertValues<typeof stripeSchema.tables.subscription>, "id">>,
    ) => {
      await orm.update("subscription", id, (b) => b.set({ ...data, updatedAt: new Date() }));
    },

    getSubscriptionByStripeId: async (stripeSubscriptionId: string) => {
      const result = await orm.findFirst("subscription", (b) =>
    ...
    getAllSubscriptions: async () => {
      return (await orm.find("subscription", (b) => b.whereIndex("primary"))).map(
        (subscription) => ({
          ...subscription,
          id: subscription.id.externalId,
          status: subscription.status as Stripe.Subscription.Status,
    ...
    syncStripeSubscription: async (referenceId: string, stripeCustomerId: string) => {
      ...
      const existingSubscription = await orm.findFirst("subscription", (b) =>
        b.whereIndex("idx_stripe_subscription_id", (eb) =>
          eb("stripeSubscriptionId", "=", stripeSubscription.id),
        ),
      );
      if (existingSubscription) {
        await orm.update("subscription", existingSubscription.id, (b) =>
          b.set({
