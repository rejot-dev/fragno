...
import { getId, toDate, stripeSubscriptionToInternalSubscription } from "../utils";
...
export async function customerSubscriptionDeletedHandler({
  ...
  deps.log.info(
    `Marked subscription ${(subscription as any).id} as canceled (Stripe ID: ${stripeSubscription.id})`,
  );
}
