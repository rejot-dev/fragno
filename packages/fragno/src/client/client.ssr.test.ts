...
describe("server side rendering", () => {
  ...
  test("should be able to stream data server side", async () => {
    ...
    const client = createClientBuilder(clientConfig, streamLibraryConfig);
    const clientObj = {
  ...
  test("throws FragnoClientUnknownApiError when the stream is not valid JSON", async () => {
    const streamErrorLibraryConfig = {
      name: "stream-error-library",
      routes: [
        addRoute({
          method: "GET",
    ...
    const client = createClientBuilder(clientConfig, streamErrorLibraryConfig);
    const clientObj = {
