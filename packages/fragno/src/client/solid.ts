...
function createSolidStore<const T extends object>(hook: FragnoStoreData<T>): FragnoSolidStore<T> {
  // If the store object itself is a single atom, wrap it with useStore
  if (isReadableAtom(hook.obj)) {
    // TODO: remove this type cast
    return useStore(hook.obj) as FragnoSolidStore<T>;
  }
  ...
  return (() => result) as FragnoSolidStore<T>;
}
