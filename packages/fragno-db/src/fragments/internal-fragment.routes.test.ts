...
describe("internal fragment describe routes", () => {
  const setupAdapter = async ({ outboxEnabled = false, migrateInternal = true } = {}) => {
    ...
    const adapter = new SqlAdapter({
      ...
      outbox: outboxEnabled ? { enabled: true } : undefined,
    });
  ...
  it("exposes the outbox route only when enabled", async () => {
    ...
    const alphaFragment = instantiate(alphaDef)
      .withOptions({ databaseAdapter: outboxAdapter, mountRoute: "/alpha" })
      .build();
    ...
    const betaDef = defineFragment("beta-fragment").extend(withDatabase(betaSchema)).build();
    const betaFragment = instantiate(betaDef)
      .withOptions({ databaseAdapter: noOutboxAdapter, mountRoute: "/beta" })
      .build();

    const noOutboxResponse = await betaFragment.callRouteRaw("GET", "/_internal/outbox" as never);
