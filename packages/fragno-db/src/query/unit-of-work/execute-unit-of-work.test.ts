...
import { schema, idColumn, referenceColumn, FragnoId } from "../../schema/create";
import {
  createUnitOfWork,
...
const relatedTablesSchema = schema((s) =>
  s
    .addTable("users", (t) =>
...
describe("executeRestrictedUnitOfWork", () => {
  ...
  describe("executeTxCallbacks", () => {
    it("should execute retrieve and mutate callbacks in order", async () => {
      ...
      const result = await executeTxCallbacks(
        {
          retrieve: ({ forSchema }) => {
            ...
            return forSchema(testSchema).find("users", (b) => b.whereIndex("idx_email"));
          },
          mutate: ({ forSchema }, [users]) => {
            executionOrder.push("mutate");
            const uow = forSchema(testSchema);
            uow.create("users", { email: "new@example.com", name: "New", balance: 0 });
            return { originalUser: users[0]?.email };
          },
        },
        {
          createUnitOfWork: () => createUnitOfWork(compiler, executor, decoder),
      ...
      expect(result).toEqual({ originalUser: "test@example.com" });
    });

    it("should provide nonce and currentAttempt in context", async () => {
      const compiler = createMockCompiler();
      const executor: UOWExecutor<unknown, unknown> = {
        executeRetrievalPhase: async () => [[]],
        executeMutationPhase: async () => ({
      ...
      await executeTxCallbacks(
        {
          retrieve: ({ forSchema, nonce, currentAttempt }) => {
            capturedNonce = nonce;
    ...
    it("should handle retrieve-only transactions", async () => {
      ...
      // When there's no mutate callback, result is undefined
      const result = await executeTxCallbacks(
        {
          retrieve: ({ forSchema }) => {
            return forSchema(testSchema).find("users", (b) => b.whereIndex("idx_email"));
    ...
    it("should support chained creates in mutate-only callback where second entity references first", async () => {
      const compiler = createMockCompiler();
      const executor: UOWExecutor<unknown, unknown> = {
        executeRetrievalPhase: async () => [],
        executeMutationPhase: async () => ({
          success: true,
          createdInternalIds: [BigInt(1), BigInt(2)],
        }),
      };
      const decoder = createMockDecoder();
      ...
      const result = await executeTxCallbacks(
        {
          mutate: ({ forSchema }) => {
            const uow = forSchema(relatedTablesSchema);

            // First create returns a FragnoId synchronously
            const userId = uow.create("users", {
              name: "John Doe",
              email: "john@example.com",
            });

            // Use that ID immediately in a second create
            const profileId = uow.create("profiles", {
              user_id: userId,
              bio: "Software engineer",
            });

            // Return both IDs - they're available synchronously
      ...
      // Both IDs should be valid FragnoId instances
      expect(result.userId).toBeInstanceOf(FragnoId);
      expect(result.profileId).toBeInstanceOf(FragnoId);

      // IDs should have been generated
      expect(result.userId.externalId).toBeDefined();
      expect(result.profileId.externalId).toBeDefined();

      // IDs should be different
      expect(result.userId.externalId).not.toBe(result.profileId.externalId);
    });

    it("should support chaining service method calls in mutate-only callback, similar to handler pattern", async () => {
      const compiler = createMockCompiler();
      const executor: UOWExecutor<unknown, unknown> = {
        executeRetrievalPhase: async () => [],
        executeMutationPhase: async () => ({
      ...
      // Simulate service methods that schedule mutations synchronously
      // (like createUser and createProfile when designed for composition)
      const createUserService = (uow: TypedUnitOfWork<typeof relatedTablesSchema>) => {
        return uow.create("users", {
          name: "John Doe",
          email: "john@example.com",
        });
      };
      ...
      const createProfileService = (
        uow: TypedUnitOfWork<typeof relatedTablesSchema>,
        userId: FragnoId,
      ) => {
      ...
      const result = await executeTxCallbacks(
        {
          mutate: ({ forSchema }) => {
            ...
            // Call service methods synchronously - they return IDs immediately
            const userId = createUserService(uow);
            const profileId = createProfileService(uow, userId);

            // Return both IDs - pattern matches the commented code in db-fragment-integration.test.ts
            return { userId, profileId };
          },
        },
        {
      ...
      // Verify both IDs are returned correctly
      expect(result.userId).toBeInstanceOf(FragnoId);
      expect(result.profileId).toBeInstanceOf(FragnoId);
      expect(result.userId.externalId).toBeDefined();
      expect(result.profileId.externalId).toBeDefined();
    ...
    it("should retry on concurrency conflict", async () => {
      ...
      const result = await executeTxCallbacks(
        {
          mutate: (_context, _results) => ({ value: "result" }),
        },
        {
          createUnitOfWork: () => createUnitOfWork(compiler, executor, decoder),
          retryPolicy: new ExponentialBackoffRetryPolicy({ maxRetries: 3, initialDelayMs: 1 }),
    ...
    it("should increment currentAttempt on retries", async () => {
      const compiler = createMockCompiler();
      let attemptCount = 0;
      ...
      await executeTxCallbacks(
        {
          retrieve: ({ forSchema, currentAttempt }) => {
            capturedAttempts.push(currentAttempt);
            return forSchema(testSchema).find("users", (b) => b.whereIndex("idx_email"));
          },
        },
        {
      ...
      expect(capturedAttempts).toEqual([0, 1, 2]);
    });
  });
  ...
  describe("executeTxArray with executeServiceTx", () => {
    ...
    it("should expose retrievalResult after retrieval phase, before mutations complete", async () => {
      const compiler = createMockCompiler();
      const mockUser = {
        id: FragnoId.fromExternal("1", 1),
        email: "user1@example.com",
      ...
      // Start service tx - returns a ServiceTxResult with retrievalResult property
      const serviceResult = executeServiceTx(
        testSchema,
        {
          retrieve: (uow) => {
            return uow.findFirst("users", (b) => b.whereIndex("idx_email"));
          ...
          mutate: async (uow, [user]) => {
            ...
            uow.update("users", user.id, (b) => b.set({ balance: user.balance + 50 }).check());
            return { ok: true as const, newBalance: user.balance + 50 };
          },
        },
        restrictedUow,
      );
      ...
      // retrievalResult promise resolves to the actual retrieval data (NOT the mutation result)
      expect(await serviceResult.retrievalResult).toEqual([mockUser]);

      // Execute mutations
      await baseUow.executeMutations();

      // Awaiting the full result gives the mutation result
      const finalResult = await serviceResult;
      expect(finalResult).toEqual({ ok: true, newBalance: 150 });
    });

    it("should reject retrievalResult when retrieval phase throws", async () => {
      const compiler = createMockCompiler();
      ...
      // Execute retrieval phase - this will throw
      await expect(baseUow.executeRetrieve()).rejects.toThrow("Database connection failed");
    });

    it("should reject retrievalResult when retrieve callback throws synchronously", async () => {
      const compiler = createMockCompiler();
      const executor: UOWExecutor<unknown, unknown> = {
        executeRetrievalPhase: async () => [[]],
        executeMutationPhase: async () => ({
      ...
      const syncError = new Error("Sync error in retrieve callback");

      // Start service tx with a retrieve callback that throws synchronously
      const serviceResult = executeServiceTx(
        testSchema,
        {
          retrieve: () => {
      ...
      // baseUow.executeRetrieve() resolves - error is contained in service promises
      // But we must handle all rejected promises in the same tick to avoid unhandled rejection
      await Promise.all([
        baseUow.executeRetrieve(),
        expect(serviceResult.retrievalResult).rejects.toThrow("Sync error in retrieve callback"),
        expect(serviceResult).rejects.toThrow("Sync error in retrieve callback"),
      ]);
    });
  });

  describe("executeTxWithDeps with executeServiceTx", () => {
    it("should use executeServiceTx result as dependency in executeTxWithDeps", async () => {
      const compiler = createMockCompiler();
      const executor: UOWExecutor<unknown, unknown> = {
        executeRetrievalPhase: async () => [
          [
      ...
      // Execute using executeTxWithDeps with executeServiceTx as dependency
      const result = await executeTxWithDeps(
        ...
        ],
        {
          mutate: (context, [[user]]) => {
            ...
            // Use the retrieved user in a mutation
            context
              .forSchema(testSchema)
              .update("users", user.id, (b) => b.set({ balance: user.balance + 100 }));
            return { ok: true as const, userId: user.id, newBalance: user.balance + 100 };
          },
        },
        {
          createUnitOfWork: () => {
      ...
      expect(result).toEqual({
        ok: true,
        userId: FragnoId.fromExternal("1", 1),
        newBalance: 200,
      });
    });

    it("should use multiple executeServiceTx results as dependencies", async () => {
      const compiler = createMockCompiler();
      const executor: UOWExecutor<unknown, unknown> = {
        executeRetrievalPhase: async () => [
          // First service retrieval result
          [
            {
              id: FragnoId.fromExternal("1", 1),
              email: "user1@example.com",
          ...
          // Second service retrieval result
          [
            {
      ...
      // Execute using executeTxWithDeps with multiple executeServiceTx as dependencies
      const result = await executeTxWithDeps(
        () => [
          executeServiceTx(
            testSchema,
            {
              retrieve: (uow) => uow.findFirst("users", (b) => b.whereIndex("idx_email")),
            },
            currentUow!,
          ),
          executeServiceTx(
        ...
        {
          mutate: (context, [[user1], [user2]]) => {
            if (!user1 || !user2) {
              return { ok: false as const };
            }
            // Transfer balance from user1 to user2
            const uow = context.forSchema(testSchema);
            uow.update("users", user1.id, (b) => b.set({ balance: user1.balance - 50 }));
            uow.update("users", user2.id, (b) => b.set({ balance: user2.balance + 50 }));
            return {
              ok: true as const,
              user1NewBalance: user1.balance - 50,
              user2NewBalance: user2.balance + 50,
            };
          },
        },
        {
    ...
    it("should work with executeServiceTx that has both retrieve and mutate", async () => {
      const compiler = createMockCompiler();
      const executor: UOWExecutor<unknown, unknown> = {
        executeRetrievalPhase: async () => [
          [
      ...
      // The dep service has both retrieve and mutate
      const result = await executeTxWithDeps(
        () => [
          executeServiceTx(
            testSchema,
            {
              ...
              mutate: (uow, [user]) => {
                ...
                // This mutation is scheduled as part of the dep
                uow.update("users", user.id, (b) => b.set({ name: "Updated by dep" }));
                return { processed: true as const, userId: user.id };
              },
            },
            currentUow!,
          ),
        ...
        {
          mutate: (context, [retrievalResults]) => {
            // deps now correctly contains retrieval results from each service
            const [user] = retrievalResults;
            ...
            if (!user) {
              return { combined: false as const };
            }
            // Add our own mutation on top
            context
              .forSchema(testSchema)
              .update("users", user.id, (b) => b.set({ balance: user.balance + 500 }));
            return { combined: true as const, userId: user.id };
          },
        },
        {
          createUnitOfWork: () => {
          ...
          executeServiceTx(
            testSchema,
            {
              ...
              mutate: (uow, [user]) => {
                ...
                return { processed: true as const, userId: user.id, newBalance };
              },
            },
            currentUow!,
          ),
...
// =============================================================================
// NEW UNIFIED TX API TESTS
// =============================================================================

import { createServiceTx, executeTx, isTxResult, type TxResult } from "./execute-unit-of-work";
