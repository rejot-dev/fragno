...
import type { SupportedDatabase, SQLiteProfile } from "../generic-sql/driver-config";
import {
...
function getPrismaScalarType(
  ...
  switch (column.type) {
    ...
    case "integer":
      return { type: "Int" };
    case "bigint":
      if (provider === "sqlite" && sqliteProfile === "fragno") {
        return { type: "Bytes" };
      }
      return { type: "BigInt" };
    case "bool":
...
function getColumnDefault(
  ...
  if ("dbSpecial" in column.default && column.default.dbSpecial === "now") {
    const scalar = getPrismaScalarType(column, provider, sqliteProfile);
...
function generateColumnFields(
  ...
  for (const column of Object.values(table.columns)) {
    ...
    const defaultValue = getColumnDefault(column, provider, sqliteProfile);
    if (defaultValue) {
...
function generateModel(
  ...
  const fieldLines = [
    ...generateColumnFields(table, provider, sqliteProfile, fieldNameByColumn),
...
export function generateSchema(
  ...
  const sqliteProfile = options?.sqliteProfile ?? (provider === "sqlite" ? "prisma" : "fragno");
