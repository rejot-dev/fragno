  ...
  override compileUpsert(table: AnyTable, options: UpsertCompilerOptions) {
    ...
    if (options.conflictAction === "ignore") {
      ...
      insert = insert.onDuplicateKeyUpdate({
        [idColumnName]: sql.ref(idColumnName),
      });
    ...
    if (options.checkVersionValue !== undefined) {
      const updateCondition = sql`${sql.ref(versionColumnName)} = ${options.checkVersionValue}`;
      for (const [columnName, value] of Object.entries(updateValues)) {
        updateValues[columnName] = sql`if(${updateCondition}, ${value}, ${sql.ref(columnName)})`;
      }
    }
