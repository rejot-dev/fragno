...
describe("PreparedMigrations - Multi-step Migration Scenarios", () => {
  ...
  test("PostgreSQL: migration 0 -> 3 (full schema with posts and FK)", () => {
    ...
    expect(sql).toMatchInlineSnapshot(`
      ...
      insert into "fragno_db_settings" ("id", "key", "value", "_shard") values ('BflimUWc1NbCMMDD9SM3gQ', 'test.schema_version', '3', '__fragno_global__');"
    `);
  });
  ...
  test("SQLite: migration 1 -> 2 (make column nullable)", () => {
    ...
    expect(sql).toMatchInlineSnapshot(`
      ...
      create table "users_nullable__fragno_tmp_414fdd" ("id" text not null unique, "name" text, "_internalId" integer not null primary key autoincrement, "_version" integer default 0 not null, "_shard" text default '__fragno_global__' not null);
...
describe("PreparedMigrations - default system migrations", () => {
  ...
  test("mysql system migrations guard non-idempotent DDL with information_schema checks", () => {
    const prepared = createPreparedMigrations({
      schema: testSchema,
      namespace: "test",
      database: "mysql",
    ...
    expect(sql).toContain("information_schema.columns");
    expect(sql).toContain("information_schema.statistics");
    expect(sql).toContain("fragno_db_system_migration_stmt");
  });
});
