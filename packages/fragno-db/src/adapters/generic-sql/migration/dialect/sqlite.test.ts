...
describe("SQLiteSQLGenerator", () => {
  ...
  describe("alter-table - update-column", () => {
    ...
    it("should preserve rename and drop operations during recreation", () => {
      const operation: MigrationOperation = {
        type: "alter-table",
        name: "test_table",
        value: [
        ...
        metadata: {
          recreateTable: {
            ...
            copyColumns: ["id", "title", "old_field", "_internalId", "_version"],
            indexes: [
              {
                name: "idx_title",
                columns: ["title"],
  ...
  describe("SQLite FK preprocessing", () => {
    ...
    it("should preserve create-table metadata when merging inline foreign keys", () => {
      const operations: MigrationOperation[] = [
        {
          type: "create-table",
          name: "posts",
          columns: [
            { name: "id", type: "integer", isNullable: false, role: "internal-id" },
            { name: "author_id", type: "integer", isNullable: false, role: "reference" },
            { name: "org_id", type: "integer", isNullable: false, role: "reference" },
          ],
          metadata: {
            preserve: "keep",
            inlineForeignKeys: [
              {
                name: "posts_orgs_org_fk",
                columns: ["org_id"],
      ...
      if (preprocessed[1].type === "create-table") {
        const metadata = preprocessed[1].metadata as {
          preserve?: string;
          inlineForeignKeys?: {
            name: string;
            columns: string[];
