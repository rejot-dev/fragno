...
describe("resolveSystemMigrationTables", () => {
  const versionedSchema = schema("test", (s) => {
    return s
      .addTable("users", (t) => {
        return t.addColumn("id", idColumn()).addColumn("name", column("string"));
      })
  ...
  it("scopes tables to operations applied up to fromVersion", () => {
    expect(Object.keys(resolveSystemMigrationTables(versionedSchema, 0))).toEqual([]);
    expect(Object.keys(resolveSystemMigrationTables(versionedSchema, 1))).toEqual(["users"]);
    expect(Object.keys(resolveSystemMigrationTables(versionedSchema, 2))).toEqual(["users"]);
    expect(Object.keys(resolveSystemMigrationTables(versionedSchema, 3))).toEqual([
      "users",
      "posts",
    ]);
  });
