...
>(databases: TDatabases): Promise<ExecuteMigrationResult[]> {
  ...
  if (compiledSettingsMigration.statements.length > 0) {
    await settingsPreparedMigrations.execute(settingsSourceVersion, settingsTargetVersion, {
    ...
    executedNamespaceKeys.add(SETTINGS_TABLE_NAME);
  }
  ...
  // 3. Execute all fragment migrations in order
