...
type TestHookMap = {
  onTest: (payload: { ok: boolean }) => void | Promise<void>;
  onTestUsesBoundService: (payload: {}) => void | Promise<void>;
};
...
describe("createDurableHooksProcessor", () => {
  ...
  it("allows bound fragment services to run inside durable hooks", async () => {
    ...
    await fragment.inContext(async function () {
      await this.handlerTx()
        .mutate(({ forSchema }) => {
          const uow = forSchema<typeof testSchema, TestHookMap>(testSchema);
          uow.create("items", {
