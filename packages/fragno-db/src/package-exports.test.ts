...
describe("@fragno-dev/db package exports", () => {
  it("keeps browser bundles on the browser build even when workerd is present", () => {
    expect(
      resolveExportTarget(
        packageJson.exports["."],
        new Set(["workerd", "worker", "browser", "module", "development"]),
      ),
    ).toBe("./dist/browser/mod.js");
  });
  ...
  it("keeps ./sync server-side when SSR resolution also exposes browser", () => {
    expect(
      resolveExportTarget(
        packageJson.exports["./sync"],
        new Set(["node", "browser", "module", "development"]),
      ),
    ).toBe("./dist/sync/index.js");
  });
});
