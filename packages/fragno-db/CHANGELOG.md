...
- d2da73f: feat: add atomic bulk deletes with optimistic concurrency and bounded SQL statements.
- 187d67e: feat: replace bare outbox streaming with protocol-1 frames and migrate built-in consumers.
...
- fcd0179: fix: batch normalized outbox mutations to stay within Durable Object SQLite binding limits.
- 4db850e: perf: stream bounded outbox pages item by item with database backpressure, isolate bounded catch-up
  work, and share one live polling loop across active stream observers.
- 766f977: perf: bound live workflow emission memory and remove snapshot APIs.
...
- 26a21e3: feat: expose request-source and Durable Object initialization instrumentation.
...
- c1255e4: fix: await durable hook alarm handoff after mutations, fail runtime initialization when dispatcher
  alarm setup fails, await async error observers, and keep hook processing owned by alarm invocations.
- 318921d: fix: process durable hooks concurrently with bounded backpressure.
- 7e2d989: refactor: reject Durable Object SQLite introspection explicitly.
- d62cc57: feat: include execution duration in Durable Object query metrics.
- ae415ff: fix: retry workflow step inserts when Durable Object query errors wrap SQLite unique conflicts.
- 204bbb7: feat: expose final Durable Object SQLite row counters through explicit query instrumentation.
...
- c41c1e2: fix: preserve JSON value types in joined query-tree projections.
...
- 1c57f29: perf: reduce idle outbox polling and name internal database transaction spans.
...
- a61b3a5: fix: parse Cloudflare SQLite unique-constraint columns without trailing result metadata.
