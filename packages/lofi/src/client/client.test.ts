...
describe("LofiClient", () => {
  it("drains pages and advances the cursor", async () => {
    ...
    const applied: Array<{ sourceKey: string; versionstamp: string }> = [];

    const client = new LofiClient({
      outboxUrl: baseOutboxUrl,
      endpointName: "app-outbox",
      adapter: {
        applyOutboxEntry: async ({ sourceKey, versionstamp }) => {
          applied.push({ sourceKey, versionstamp });
          return { applied: true };
        },
    ...
    expect(applied).toEqual([
      { sourceKey: "app-outbox::outbox", versionstamp: "vs-1" },
      { sourceKey: "app-outbox::outbox", versionstamp: "vs-2" },
      { sourceKey: "app-outbox::outbox", versionstamp: "vs-3" },
    ]);
