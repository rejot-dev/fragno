...
<Callout type="warn" title="File Name">
  Make sure the field name does **not** end with `.client.ts`. In some frameworks, this makes it so
  that the file cannot be loaded on the server at all. Meaning it cannot be imported during server
  render.
</Callout>
