---
title: Middleware
description: Learn how to use middleware in Fragno
---
...
Middleware is defined as part of the Fragment's server configuration. It's called for every request
and can modify the request before it reaches the route handlers. Fragno only supports a single
middleware method, it's not possible to add multiple middleware handlers like in other frameworks.
...
parameters, query parameters, and more. The second argument contains the output context, which can
be used create responses.
...
<Callout type="warn">
  Request body and query parameters currently cannot be modified in middleware. See [Issue
...
for parameters and request input. The callback passed to `ifMatchesRoute` is only executed when that
route is matched.
