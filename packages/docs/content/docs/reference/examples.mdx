...
export function createTodos(config: TodosConfig = {}) {
  return createLibrary(todosDefinition, config, [
    listTodosRoute,
    createTodoRoute,
    updateTodoRoute,
    deleteTodoRoute,
...
const chatDefinition = defineLibrary<ChatConfig>("chat-fragment").withDependencies((config) => ({
...
export function createChat(config: ChatConfig) {
  return createLibrary(chatDefinition, config, [chatRoute]);
}
...
const storageDefinition = defineLibrary<StorageConfig>("storage-fragment").withDependencies(
  (config) => ({
...
export function createStorage(config: StorageConfig) {
  return createLibrary(storageDefinition, config, [uploadRoute, listFilesRoute]);
}
...
import { createTodosClient } from "@/lib/todos-fragment";
...
const { useTodos, useCreateTodo, useUpdateTodo, useDeleteTodo } = useFragno(todosClient);
