...
There are a number of concepts you should be familiar with before publishing a Fragment:
...
But before getting into that, we'll start with the basics and ignore the details for now. In this
walkthrough, we'll create two simple server routes for getting and creating todos, then we'll also
create a client-side Fragment that provides TanStack Query-style hooks for the two routes.
...
method, this method has access to the config object.
...
`inputSchema`, `outputSchema`, `handler`, `errorCodes`, and `queryParameters`.

The first argument passed to the handler is the input context object, this object has an `input`
field with a `valid` method that can be used to validate the input. It also contains other fields
related to the request.

The second argument passed to the handler is the output context object, this object contains a
number of method fields: `json`, `jsonStream`, `empty`, and `error`. These methods can be used to
return a response.
...
// Dependencies and services can be defined on a per-route level, since they might be a subset
// of all dependencies/services.
...
instance. We pass the library definition, the config, and the routes to the `createLibrary`
function.
...
export function createTodos(config: TodosConfig, fragnoConfig: FragnoPublicConfig = {}) {
  return createLibrary(todosDefinition, config, [getTodosRoute, addTodoRoute], fragnoConfig);
}
```
...
we simply create hooks for the two routes we defined.
...
We went over building a basic Fragment. In the next section, we'll go over the advanced features of
Fragno. It's also important to have a look at the [Building & Publishing](./building-publishing)
page.
