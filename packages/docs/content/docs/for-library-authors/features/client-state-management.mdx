...
An important feature of Fragno is the ability to also define client-side components as part of the
Fragment. Under the hood, Fragno uses [Nanostores](https://github.com/nanostores/nanostores) to
manage state. These reactive primitives map cleanly onto many popular frontend frameworks, such as
React, Svelte, Vue, and of course, vanilla JavaScript. Helper functions are provided to
automatically create reactive stores for each API route. These are modelled after the popular
...
instantiated with the library definition, the public config, and the routes. It is called as part of
...
point. It contains the hooks and mutators for the routes, as well as any other arbitrary values that
you want to expose to the user on the client-side. The object is passed through the
framework-specific `useFragno` helper function, which returns the hooks and mutators as reactive
stores for the framework.
...
<AutoTypeTable path="fragno/src/client/client.ts" name="CreateHookOptions" />

### Fragment User Usage

Your users will use this hook by calling the function with an object containing `path` and `query`
fields. These are based on the route's path and query parameters. The function returns an object
...
As we covered in the [Route Definition](./route-definition.mdx) section, Fragno supports streaming
...
The `createMutator` method takes an optional third parameter that is a function that will be called
when the mutation has succeeded. It is passed the `invalidate` function, which can be used to
invalidate other routes.

When no `onInvalidate` function is provided, it defaults to the following implementation:
...
<Callout title="Invalidation only happens on exactly the same route/path" type="warning">
  In this example, updating or deleting a todo would not invalidate the `useTodos` hook. So we'd
  have to specify a custom `onInvalidate` function.
</Callout>

<Callout title="Default behaviour might be removed in the future" type="warning">
  This default behaviour might be removed in the future as we cannot automatically infer which
...
The resulting data is also returned from the `mutate` function, but generally it'd be read from the
...
Same as with the `createHook` method, when a route returns a streaming response, the `loading` state
...
reactively. In this example, we **must** call `builder.createStore` to make sure the aggregated
message can be transformed into a reactive store for the framework the user is using.
...
the aggregation in the background. Instead of giving the user a `mutate` function, we've given them
a more descriptive `sendMessage` function.

We can return any other arbitrary values or functions from the `createMyFragmentClient` function.

<Callout title="`createStore` nesting" type="warning">
  If an object is passed to `createStore`, all fields on this object will be attempted to be made
  reactive. In the case of a function, nothing will happen to it. Deeply nested keys will **not** be
  considered.
</Callout>
