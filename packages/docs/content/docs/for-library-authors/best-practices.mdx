...
```typescript
// ✅ Good
const userManagementDefinition = defineLibrary<UserConfig>("user-management");
...
// ❌ Bad
const libDef = defineLibrary<Config>("lib");
...
const myFragmentDefinition = defineLibrary<MyFragmentConfig>("my-fragment").withDependencies(
  (config) => ({
