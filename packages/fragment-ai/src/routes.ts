      ...
      defineRoute({
        ...
        handler: async function ({ pathParams, input }, { jsonStream, error }) {
          ...
          return jsonStream(async (stream) => {
            ...
            let allowRetry = true;
            let piAiToolCalls: ExecutedToolCall[] | null = null;
            ...
            const emitPiAiToolCalls = async (toolCalls: ExecutedToolCall[]) => {
                  ...
                  for await (const event of responseStream) {
                    ...
                    if (eventType === "response.output_text.delta") {
                      ...
                      if (item && isToolCallType(item["type"])) {
                        const toolCallId = resolveToolCallIdFromItem(
                          item,
                          event,
                          toolCallIdByCallId,
                        );
                ...
                try {
                  ...
                  for await (const event of responseStream) {
                    if (event.type === "text_delta") {
                      ...
                      recordRunEventIfEnabled("output.text.delta", { delta: event.delta });
                    }

                    if (event.type === "error") {
                      finalStatus = event.reason === "aborted" ? "cancelled" : "failed";
                      errorMessage = event.error.errorMessage ?? "AI request failed";
                  ...
                  if (finalMessage.stopReason === "toolUse") {
                    ...
                    piAiToolCalls = toolLoop.toolCalls;
                    if (piAiToolCalls.length > 0) {
                      await emitPiAiToolCalls(piAiToolCalls);
                    }

                    if (toolLoop.error) {
                      finalStatus = "failed";
                      errorMessage = toolLoop.error;
                      allowRetry = false;
                    } else if (toolLoop.message.stopReason === "error") {
                      finalStatus = "failed";
                      errorMessage = toolLoop.message.errorMessage ?? "AI request failed";
                      ...
                      if (finalText) {
                        ...
                        recordRunEvent("output.text.done", { text: finalText });
                      }
                    }
                  } else if (finalMessage.stopReason === "error") {
                    finalStatus = "failed";
                    errorMessage = finalMessage.errorMessage ?? "AI request failed";
                  } else if (finalMessage.stopReason === "aborted") {
                    finalStatus = "cancelled";
                    errorMessage = null;
                  } else {
