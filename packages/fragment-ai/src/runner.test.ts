...
describe("AI Fragment Runner", () => {
  ...
  test("runner tick should record pi-ai tool calls when unsupported", async () => {
    ...
    expect(storedRun?.error).toBe("TOOL_CALL_UNSUPPORTED");
  ...
  test("runner tick should execute pi-agent-core tool calls when registry provided", async () => {
    const toolExecute = vi.fn(async (_toolCallId: string, params: unknown) => {
      const args =
        params && typeof params === "object" && !Array.isArray(params)
          ? (params as Record<string, unknown>)
          : {};
      const content = [{ type: "text" as const, text: "Tool result response" }];
      return {
        content,
        details: { ok: true, query: args["query"] },
      };
    });
    const customConfig = {
      defaultModel: { id: "claude-test", provider: "anthropic" },
      ...
      retries: { baseDelayMs: 1000 },
      toolRegistry: {
        tools: [{ name: "search", parameters: {}, execute: toolExecute }],
      },
    };
    ...
    const thread = await customRunService<{ id: string }>(() =>
      customFragment.services.createThread({ title: "Pi-Agent Tool Thread" }),
    );
    ...
    expect(toolCalls[0]?.isError).toBe(0);
    expect(toolCalls[0]?.result).toMatchObject({ ok: true, query: "hello" });
    expect(toolExecute).toHaveBeenCalledWith("call_tool_1", { query: "hello" });
