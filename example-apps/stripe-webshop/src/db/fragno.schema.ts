import { pgTable, varchar, text, bigserial, integer, uniqueIndex, timestamp, boolean, index, json, bigint, foreignKey } from "drizzle-orm/pg-core"
...
export const form_forms = pgTable("form_forms", {
  id: varchar("id", { length: 30 }).notNull().$defaultFn(() => createId()),
  ...
  index("idx_form_slug_forms").on(table.slug)
])

export const response_forms = pgTable("response_forms", {
  id: varchar("id", { length: 30 }).notNull().$defaultFn(() => createId()),
