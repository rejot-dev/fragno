  ...
  return (
    <div class="space-y-6 rounded-lg bg-white p-6 shadow">
      <Suspense fallback={<div>Loading...</div>}>
        <div>
          Cannot use Suspend in Solid with our hooks at the moment. Both of these signals should
      ...
      <button
        ...
        onClick={() => mutate({ body: { message: (mutateData() || data() || "") + "!" } })}
        disabled={mutateLoading()}
      >
        Change data: {mutateData()}
      </button>
    </div>
  );
}
