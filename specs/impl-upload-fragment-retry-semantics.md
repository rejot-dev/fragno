...
- [ ] Update `packages/fragment-upload/src/schema.ts` to add a mirrored copy of file metadata +
...
- [ ] Update upload services in `packages/fragment-upload/src/services/uploads.ts` to create only
...
- [ ] Update upload failure/abort/timeout flows (`markUploadFailed`, `markUploadAborted`,
...
- [ ] Update upload routes in `packages/fragment-upload/src/routes/uploads.ts` to use upload
...
- [ ] Update the `POST /files` route in `packages/fragment-upload/src/routes/files.ts` to run the
...
- [ ] Update shared types and schemas (`packages/fragment-upload/src/types.ts`,
...
- [ ] Add/adjust tests to cover: idempotent upload reuse only when checksum+metadata match,
...
- [ ] Update upload docs (`apps/docs/content/docs/upload/*`) to explain optional checksums on
...
- [ ] Update `packages/fragment-upload/SMOKE_TEST_PLAN.md` to reflect the new retry and failure
