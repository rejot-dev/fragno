...
- [x] Update `DatabaseAdapter` interface to remove `createSchemaGenerator` and add any SQL adapter
      metadata needed for schema output selection (Spec 6.4, 7.1).
- [x] Rename or replace `GenericSQLAdapter` with `SqlAdapter`, including `SqlAdapterOptions` and
      SQLite profile mapping (`sqliteProfile` + `sqliteProfiles`), and bump adapter name/version to
      `sql@1` (Spec 6.1, 6.2, 7.1).
- [x] Introduce `SchemaOutputFormat` and update the generation engine to distinguish **schema
      generation** (`drizzle`/`prisma`) from **migration generation** (`sql`), enforcing `sql`-only
      `fromVersion/toVersion` rules (Spec 6.4, 7.2).
- [x] Move Drizzle and Prisma generators to `@fragno-dev/db/schema-output/*` and update imports in
      tests and internal code (Spec 6.6, 7.3).
- [x] Update `fragno-cli db generate` to accept `--format`, wire it to the generation engine, and
      make the CLI wording explicitly distinguish **schema generation** vs **migration generation**
      (Spec 6.5).
- [x] Update `fragno-cli db info` to report database type and sqlite profile for SQL adapters, and
      remove `createSchemaGenerator`-based branching (Spec 6.5).
- [x] Update `fragno-cli db migrate` messaging to clarify SQL-only behavior and remove adapter
      format inference (Spec 6.5).
- [x] Refactor all adapter usages across packages, tests, and examples to use `SqlAdapter` and the
