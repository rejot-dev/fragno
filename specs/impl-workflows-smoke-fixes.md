...
- [x] Add the `maxAttempts` pre-run guard in `packages/fragment-workflows/src/runner/step.ts` and
      ensure attempts never exceed the configured maximum (Spec 4.3).
- [x] Enforce hard `waitForEvent` deadlines in `packages/fragment-workflows/src/runner/step.ts`
...
- [x] Introduce `DbNow`/`dbNow()` as a **value** type and update serialization: - conditions in
...
- [x] Map SQLSTATE `40001`/`40P01` to concurrency conflicts in
      `packages/fragno-db/src/adapters/generic-sql/generic-sql-uow-executor.ts` (Spec 4.8).
- [x] Add `pg` pool error handling in `example-apps/wf-example/app/db/db.server.ts` to reset cached
      pool/db instances (Spec 4.7).
- [x] Add/adjust tests in `packages/fragment-workflows/src/runner.test.ts`,
